...
export default function MembersPanel({ projectId }: { projectId: string }) {
  ...
  return (
    <div>
        ...
        <div className="flex flex-wrap items-end gap-2">
          ...
          <label>
            ...
            <select
              ...
              className="rounded-md border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none placeholder:text-slate-400 focus:border-slate-500 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 dark:placeholder:text-slate-500 dark:focus:border-slate-400"
            >
      ...
      <p className="mb-6 rounded-md bg-blue-50 px-3 py-2 text-xs text-blue-800 dark:bg-blue-950/40 dark:text-blue-300">
      ...
      {loading ? (
        ...
        <>
            ...
            <div className="mb-6">
              ...
              <ul className="space-y-2">
                {invitations.map((inv) => (
                  <li
                    key={inv.id}
                    className="flex flex-wrap items-center gap-3 rounded-lg border border-slate-200 bg-white p-3 text-sm dark:border-slate-800 dark:bg-slate-900"
                  >
                    <div className="flex-1">
                      <span className="font-medium">{inv.email}</span>
                      <span className="ml-2 rounded-full bg-slate-100 px-2 py-0.5 text-xs capitalize text-slate-600 dark:bg-slate-800 dark:text-slate-300">
                        {inv.role}
                      </span>
                    ...
                    <input
                      ...
                      className="min-w-0 flex-1 rounded border border-slate-200 bg-slate-50 px-2 py-1 font-mono text-xs text-slate-500 dark:text-slate-400 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-400"
                    />
                    <button
          ...
          {/* Current members */}
          <h3 className="mb-2 text-sm font-semibold text-slate-700 dark:text-slate-300">Members</h3>
          <ul className="space-y-2">
            {members.map((m) => (
              <MemberRow
                key={m.userId}
...
function MemberRow({
  ...
  return (
    <li className="rounded-lg border border-slate-200 bg-white p-3 text-sm dark:border-slate-800 dark:bg-slate-900">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <select
          ...
          className="rounded-md border border-slate-300 bg-white px-2 py-1 text-sm capitalize text-slate-900 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100"
        >
        ...
        {isOwner ? (
          ...
          <div>
            ...
            <button
              ...
              className="mr-2 rounded-md bg-slate-900 px-3 py-1 font-medium text-white disabled:opacity-50 dark:bg-slate-100 dark:text-slate-900 dark:hover:bg-white"
            >
